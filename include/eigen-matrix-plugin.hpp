...
struct scalar_rshift_op : internal::binary_op_base<lhs_t, uint8_t> {
  ...
  template <typename Packet>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Packet packetOp(Packet const& a, Packet const& b) const { return prshift(a, b); }
  template <typename Packet>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE result_t predux(Packet const& a) const { return Eigen::internal::predux(a); }
};
