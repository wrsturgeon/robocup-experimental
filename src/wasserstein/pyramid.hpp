...
inline static constexpr auto pyrsize(vision::pxidx_t w, vision::pxidx_t h)
      -> size_t {  // NOLINT(misc-no-recursion)
  return ((w != 0) && (h != 0)) ? static_cast<size_t>(w * h) +
              pyrsize(static_cast<vision::pxidx_t>(w >> 1),
                      static_cast<vision::pxidx_t>(h >> 1))
                                : 0;
}

#define PYRAMID_ERROR \
  "Pyramid class must have only 2 allocated members: _array & _up_raw"  // NOLINT(cppcoreguidelines-macro-usage)

template <vision::pxidx_t w,
          vision::pxidx_t h>  // NOLINT(cppcoreguidelines-pro-type-member-init,hicpp-member-init)
Pyramid<w, h>::Pyramid(uint8_t const* const __restrict src) {
  // Private constructor: guaranteed that by this point, src points to valid (w,
  // h) uint8_t data
...
auto Pyramid<w, h>::operator()(vision::pxidx_t x, vision::pxidx_t y)
      -> uint8_t& {
  return _array[y][x];
}
...
auto Pyramid<w, h>::up() -> up_t& {
  return *reinterpret_cast<up_t*>(
        _up_raw);  // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast)
  ...
  // public methods And keep in mind that 0-element arrays are a compiler
  // extension
}
...
void Pyramid<w, h>::build_manual() {
  ...
  // Need to verify that we have > 0 pixels to fill, else infinite loop of
  ...
  uint8_t tmp
        [h]
        [half_w];  // NOLINT(cppcoreguidelines-avoid-c-arrays,hicpp-avoid-c-arrays,modernize-avoid-c-arrays)
  for (vision::pxidx_t y = 0; y < h; ++y) {
    for (vision::pxidx_t x = 0; x < half_w; ++x) {
      twice = static_cast<vision::pxidx_t>(
            x << 1);  // TODO(wrsturgeon): verify no overflow
      a = _array[y][twice];
      b = _array[y][twice | 1];
      tmp[y][x] = ((a >> 2) > (b >> 2))
            ? a & ~1
            : b | 1;  // Set a bit to represent left/right provenience (thx
                      // Sydney)
    }
  }
  ...
  for (vision::pxidx_t y = 0; y < half_h; ++y) {
    for (vision::pxidx_t x = 0; x < half_w; ++x) {
      twice = static_cast<vision::pxidx_t>(
            y << 1);  // TODO(wrsturgeon): verify no overflow
      a = tmp[twice][x];
      b = tmp[twice | 1][x];
...
void Pyramid<w, h>::build_eigen(EigenMap<w, h> const& lower) {
  ...
  // Eigen has some obsessive beef with things that look like column vectors but
  // aren't
  if constexpr (half_w == 1) {
    ...
    auto tmp = (lower(all, seqN(i0, half_w, 2)) >> 1) +
    ...
    upper_map = (tmp(seqN(i0, half_h, 2), all) >> 1) +
          (tmp(seqN(i0 + 1, half_h, 2), all) >> 1);
